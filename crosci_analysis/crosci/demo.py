...
crosci_code_path= os.path.abspath(os.path.join(script_dir, os.pardir))
import sys
sys.path.insert(0,crosci_code_path)
...
def compute_spectrum_biomarkers(signal_matrix, sampling_frequency, frequency_range, overlap=True, method='c', bad_idxes=[], biomarkers_to_compute=['fEI','DFA','BiS']):
    ...
    for idx_frequency,frequency_bin in enumerate(frequency_bins):
        ...
        if 'DFA' in biomarkers_to_compute or 'fEI' in biomarkers_to_compute:
            ...
            (dfa_array,window_sizes,fluctuations,dfa_intercept) = DFA(amplitude_envelope,sampling_frequency,fit_interval,
                                                                         DFA_compute_interval,overlap,method,bad_idxes)
        ...
        if 'fEI' in biomarkers_to_compute:
            ...
            (fEI_outliers_removed,fEI_val,num_outliers,wAmp,wDNF) = fEI(amplitude_envelope,sampling_frequency,
                                                                        fEI_window_seconds,fEI_overlap,dfa_array,method,bad_idxes)
        ...
        if 'BiS' in biomarkers_to_compute:
            ...
            (BiS) = bistability_index(amplitude_envelope, bad_idxes)
...
if __name__ == "__main__":
    ...
    # note that fE/I will be NaN in this case, so nothing will be swhon
    ax[2].plot(x_ticks, BiS_matrix[chan_to_plot, :])
    ax[2].set_xlabel('Frequency (Hz)')
    ax[2].set_ylabel('BiS')
    ax[2].set_xticklabels(x_labels)
    ax[2].set_xticks(x_ticks, x_labels, rotation=45)
    plt.tight_layout()
    plt.show()