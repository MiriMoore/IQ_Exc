...
from .compiled.run_DFA import run_DFA
from .compiled.run_fEI import run_fEI
from PyAstronomy.pyasl import generalizedESD
from scipy.stats import expon
from scipy.optimize import differential_evolution
from .bistability import get_bistability_index

def fEI(signal, sampling_frequency, window_size_sec, window_overlap, DFA_array, method="c", bad_idxes = []):
    ...
    for ch_idx in range(num_chans):
       ...
       if method == "c":
           (w_original_amp, w_detrendedNormalizedFluctuations) = run_fEI(original_amp, sampling_frequency,
...
def bistability_index(signal, bad_idxes=[]):
    """ Calculates the bistability index of a signal
    ...
    Returns
    -------
    bistability_index: array, shape(n_channels)
        BiS values
    HLP: array, shape(n_channels)
        High-To-Low-Power ratio values
    """
    ...
    bistabilityindex_array = np.zeros(num_chans)
    bistabilityindex_array[:] = np.NAN

    HLP_array = np.zeros(num_chans)
    HLP_array[:] = np.NAN
    ...
    for ch_idx in range(num_chans):
        ...
        power = np.power(signal_for_channel, 2)
        median_power = np.median(power)
        power = power/median_power

        bistabilityindex_array[ch_idx],HLP_array[ch_idx]=get_bistability_index(power, 200)

    #now threshold values
    HLP_array[bistabilityindex_array<2.5]=np.nan
    HLP_array[np.logical_or(HLP_array<0.02,HLP_array>0.98)]=np.nan

    return (bistabilityindex_array,HLP_array)


def DFA(signal, sampling_frequency, fit_interval, compute_interval, overlap=True, method="c", bad_idxes=[]):
    ...
    if max(window_sizes) <= num_timepoints:
        for ch_idx in range(num_chans):
            ...
            if method == "c":
                [window_sizes, fluctuations_channel] = run_DFA(signal_for_channel - np.mean(signal_for_channel),
                                                               sampling_frequency, overlap, window_sizes)
