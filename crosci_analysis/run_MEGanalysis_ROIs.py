...
eeg_path = f'data/MEGs/{ROI}/'
eeg_files = os.listdir(eeg_path)
eeg_files = [eeg_file for eeg_file in eeg_files if '.gz' in eeg_file]

analysis_path = f'proc_data/MEGanalysis_{ROI}'
...
def process_file(eeg_file_name):
    ...
    analysis_file_name = eeg_file_name.replace("pkl.gz","pkl")
    analysis_file_path = os.path.join(analysis_path,analysis_file_name)

    #skip if file has already been analyzed
    if os.path.exists(analysis_file_path):
        return
    print(f"reading file {eeg_file_name}")
    with gzip.open(eeg_file_path, 'rb') as f:
        data = pickle.load(f)
        signal_matrix = data.values
        num_channels = signal_matrix.shape[1]  # Number of columns
        print(f'----------------num_channels = {num_channels}----------------')
        num_timepoints = signal_matrix.shape[0] # Number of rows
        ...
        for idx_frequency,frequency_bin in enumerate(frequency_bins):
            ...
            filtered_signal = mne.filter.filter_data(data=signal_matrix.T,sfreq=sampling_frequency,
            ...
            try:
                ...
                print(f"Error processing file biomarker {eeg_file_name}: {e}")
